...
version: 1.0.0
description: |
  Final stage of development after the feature scope, technical design, and review have been completed.
allowed-tools:
  - Bash
  - Read
  - Write
...
**NEVER WORK ON MAIN and NEVER WORK/ATTEMPT ON/TO PROD DATA**
...
- review comments that need user decision (complex fixes, false positives)
- `.planning/tasks/TODOS.md` missing and user wants to create one (ask - see Step 5.5)
- `.planning/tasks/TODOS.md` disorganized and user wants to reorganize (ask - see Step 5.5)
...
- `.planning/tasks/TODOS.md` completed-item detection (auto-mark)
...
1. Read `review/checklist.md` from this skills workspace when available. If it cannot be read, continue with the built-in review categories in this section.
...
Check if the diff touches frontend files using changed file paths:

```bash
git diff --name-only origin/<base>...HEAD
```

Treat changes to `*.tsx`, `*.jsx`, `*.vue`, `*.svelte`, `*.css`, `*.scss`, templates, views, app routes, pages, components, or public assets as frontend scope.

**If no frontend scope is detected:** Skip design review silently. No output.

**If frontend scope is detected:**

1. **Check for DESIGN.md.** If `DESIGN.md` or `design-system.md` exists in the repo root, read it. All design findings are calibrated against it — patterns blessed in DESIGN.md are not flagged. If not found, use universal design principles.

2. If `review/design-checklist.md` exists, read it. If not, apply the frontend guidance from the active agent instructions.
...
6. Save the result to `.planning/reviews/design-review-lite.md` with timestamp, status, findings count, auto-fixed count, and current commit.
...
If `review/greptile-triage.md` exists, read it and follow the fetch, filter, classify, and escalation detection steps. If it does not exist, skip this step unless the user explicitly asks to handle Greptile comments.
...
to determine pass/fail gate. Persist the result to `.planning/reviews/codex-review.md`.
...
## Step 5.5: .planning/tasks/TODOS.md (auto-update)

Cross-reference `.planning/tasks/TODOS.md` against the changes being shipped. Mark completed items automatically; prompt only if the file is missing or disorganized.

Use `.planning/tasks/TODOS.md` as the canonical project task backlog. If `review/TODOS-format.md` exists, use it as a formatting reference.

**1. Check if `.planning/tasks/TODOS.md` exists**.

**If `.planning/tasks/TODOS.md` does not exist:** Use AskUserQuestion:

- Message: "I recommend maintaining `.planning/tasks/TODOS.md` organized by component and priority, with completed items separated. Would you like to create one?"
- Options: A) Create it now, B) Skip for now
- If A: Create `.planning/tasks/TODOS.md` with a skeleton (# TODOS heading + ## Completed section). Continue to step 3.
- If B: Skip the rest of Step 5.5. Continue to Step 6.

**2. Check structure and organization:**

Read `.planning/tasks/TODOS.md` and verify it follows the recommended structure:
...
- Message: "`.planning/tasks/TODOS.md` doesn't follow the recommended structure (skill/component groupings, P0-P4 priority, Completed section). Would you like to reorganize it?"
...
- `.planning/tasks/TODOS.md: N items marked complete (item1, item2, ...). M items remaining.`
- Or: `.planning/tasks/TODOS.md: No completed items detected. M items remaining.`
- Or: `.planning/tasks/TODOS.md: Created.` / `.planning/tasks/TODOS.md: Reorganized.`

**6. Defensive:** If `.planning/tasks/TODOS.md` cannot be written (permission error, disk full), warn the user and continue. Never stop the ship workflow for a TODO backlog failure.
...
2. **Commit ordering** (earlier commits first):
   ...
   - **VERSION + CHANGELOG + `.planning/tasks/TODOS.md`:** always in the final commit
...
Create a pull request using `gh`, Always use the project's template if it exists. Otherwise use a standard template that includes:
...
- [ ] I have updated the documentation to xeflect these changes, if applicable.
